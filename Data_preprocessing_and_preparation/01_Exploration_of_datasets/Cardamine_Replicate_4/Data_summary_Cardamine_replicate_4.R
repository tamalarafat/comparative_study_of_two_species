...
OX_7E <- CreateSeuratObject(counts = OX_data_1E, project = "OX_7E", min.features = 200)

# Add metadata information to the seurat object
OX_7E[[c("Species", "Replicates", "Genotype", "Tissue")]] <- c("Hirsuta", "WT-OX-7", "WT", "Leaf")

generate_data_summary_plot(OX_7E, covariate = "nCount_RNA", figure_name_suffix = "_CH_REP_7", binwidth = 1000)

generate_data_summary_plot(OX_7E, covariate = "nFeature_RNA", figure_name_suffix = "_CH_REP_7", binwidth = 100)
...
ggsave(plot = VlnPlot(OX_7E, features = "percent.mt") + 
       ...
       filename = "Mitochondrial_distribution_CH_REP_7.png", width = 12, height = 12, dpi = 300)
...
ggsave(plot = VlnPlot(OX_7E, features = "percent.pt") + 
         ...
         theme(axis.title.x = element_text(size = 18, face = "bold", colour = "black"), 
               ...
               title =  element_text(size = 18, face = "bold", colour = "black"),
               legend.position = "none"), 
       filename = "Chloroplast_distribution_CH_REP_7.png", width = 12, height = 12, dpi = 300)
