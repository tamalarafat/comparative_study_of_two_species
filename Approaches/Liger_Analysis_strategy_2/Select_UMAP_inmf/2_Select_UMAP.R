...
res_dir = "/netscratch/dep_tsiantis/grp_laurent/tamal/2023/Analyses/comparative_study_of_two_species/Liger_Analyses/Liger_Analysis_strategy_2/Figure_1/Select_UMAP/inmf/"
...
for (i in c(1:length(n_neighbors))){
  for (j in c(1:length(n_seed))){
    integrated.data <- RunUMAP(integrated.data, reduction = "inmf", dims = 1:50, n.components = 2, n.neighbors = n_neighbors[i], seed.use = n_seed[j])
